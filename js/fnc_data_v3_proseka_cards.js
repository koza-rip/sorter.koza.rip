...
var ary_TitleDataCardTypes = ["Permanent", "Limited", "Colofes", "Birthday"];
...
var limited = [0,1,0,0];
var colofes = [0,0,1,0];
var birthday = [0,0,0,1];

async function cardData() {
  var characters = await getAllChars();
  var cardsJP = await getAllJPCards();
  var cardsEN = await getAllENCards();
  var cardsPerm = getAll4sPermCards();
  var cardsLim = getAll4sLimCards();
  var cardsCF = getAllCFCards();
  var cardsBday = getAllBdayCards();

  // fetch all 4s permanent cards
  function getAll4sPermCards() {
    ...
    for (var i = 0; i < cardsJPID.length; i++) {
      ...
      if (cardRarity === "rarity_4" && !prosekaLimitedCards.includes(cardID) && !prosekaColofesCards.includes(cardID)) {
  ...
  function getAll4sLimCards() {
    ...
    for (var i = 0; i < cardsJPID.length; i++) {
      ...
      if (cardRarity === "rarity_4" && prosekaLimitedCards.includes(cardID)) {
        card.push(cardID, characterID, cardRarity, cardTitle, cardType, cardUnit, cardSupportUnit);
        cardsArr.push(card);
      }
    }
    ...
    for (var i = 0; i < cardsArr.length; i++) {
      ...
      var unitAndCardTypeData = cardUnitData.concat(limited);
  ...
  // fetch all colofes cards
  function getAllCFCards() {
    let charsID = Object.keys(characters);
    let cardsJPArr = Object.keys(cardsJP);
    ...
    for (var i = 0; i < cardsJPID.length; i++) {
      ...
      if (cardRarity === "rarity_4" && prosekaColofesCards.includes(cardID)) {
        card.push(cardID, characterID, cardRarity, cardTitle, cardType, cardUnit, cardSupportUnit);
        cardsArr.push(card);
      }
    }
    ...
    for (var i = 0; i < cardsArr.length; i++) {
      ...
      var unitAndCardTypeData = cardUnitData.concat(colofes);
  ...
  function getAllBdayCards() {
    ...
    for (var i = 0; i < cardsArr.length; i++) {
      ...
      var unitAndCardTypeData = cardUnitData.concat(birthday);
