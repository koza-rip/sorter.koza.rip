...
var ary_TitleDataCardTypes = ["Permanent", "Limited", "Dreamfes", "Kirafes"];
...
var limited = [0,1,0,0];
var dreamfes = [0,0,1,0];
var kirafes = [0,0,0,1];

async function cardData() {
  var characters = await getAllChars();
  var cards = await getAllCards();
  var cardsPerm = await getAll5sPermCards();
  var cardsLim = await getAll5sLimCards();
  var cardsDF = await getAllDFCards();
  var cardsKF = await getAllKFCards();

  // fetch all 5s permanent cards
  async function getAll5sPermCards() {
    ...
    for (var i = 0; i < cardsID.length; i++) {
      ...
      if (cardRarity === 5 && cardType === "permanent") {
  ...
  // fetch all 5s limited cards
  async function getAll5sLimCards() {
    let charsID = Object.keys(characters);
    let cardsID = Object.keys(cards);
    let cardsArr = [];
    ...
    for (var i = 0; i < cardsID.length; i++) {
      ...
      if (cardRarity === 5 && cardType === "limited") {
        card.push(cardID, characterID, cardRarity, cardTitle, cardType);
        cardsArr.push(card);
      }
    }
  ...
  async function getAllDFCards() {
    ...
    for (var i = 0; i < cardsID.length; i++) {
      ...
      if (cardRarity === 5 && cardType === "dreamfes") {
        card.push(cardID, characterID, cardRarity, cardTitle, cardType);
        cardsArr.push(card);
      }
    }
  ...
  async function getAllKFCards() {
    ...
    for (var i = 0; i < cardsID.length; i++) {
      ...
      if (cardRarity === 5 && cardType === "kirafes") {
        card.push(cardID, characterID, cardRarity, cardTitle, cardType);
        cardsArr.push(card);
      }
    }
